...
struct LlamaConfig {
    ...
    init(metadata: [String: GGUFValue]) throws {
        ...
        self.vocabSize = Int(try getRequiredUInt64("tokenizer.ggml.vocab_size")) // Verify key with actual file
        self.sequenceLength = Int(try getRequiredUInt64("llama.context_length")) // Verify key with actual file
        self.rmsNormEps = try getRequiredFloat32("llama.attention.layer_norm_rms_epsilon") // Verify key
        ...
        self.ropeDimensionCount = Int(metadata["llama.rope.dimension_count"]?.uint64 ?? UInt64(embeddingDim / numHeads)) // Often defaults to head dimension
        self.ropeFreqBase = getOptionalFloat32("llama.rope.freq_base", default: 10000.0) // Common default

        // --- Validations and Derived Values ---
        guard embeddingDim % numHeads == 0 else {
            throw ConfigError.configurationError(message: "Embedding dimension (\(embeddingDim)) must be divisible by number of heads (\(numHeads)).")
        }
        self.headDim = embeddingDim / numHeads

        guard numHeads % numKeyValueHeads == 0 else {
             throw ConfigError.configurationError(message: "Number of heads (\(numHeads)) must be divisible by number of key/value heads (\(numKeyValueHeads)).")
         }

        // Print loaded config for verification during development
        print("--- LlamaConfig Initialized ---")
        print(String(format: "  Layers: %d, Embed Dim: %d, Hidden Dim: %d", numLayers, embeddingDim, hiddenDim))
        print(String(format: "  Heads: %d, KV Heads: %d, Head Dim: %d, Query Groups: %d", numHeads, numKeyValueHeads, headDim, numQueryGroups))
        print(String(format: "  Vocab: %d, Seq Len: %d, Norm Eps: %.8f", vocabSize, sequenceLength, rmsNormEps))
        print(String(format: "  RoPE Dim: %d, RoPE Freq Base: %.1f", ropeDimensionCount, ropeFreqBase))
        print("-----------------------------")
    }
}
...
class LlamaModel {
    ...
    {
        precondition(blocks.count == config.numLayers, "Number of blocks (\(blocks.count)) must match config.numLayers (\(config.numLayers))")
        self.config = config
